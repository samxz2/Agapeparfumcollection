...
// Función para obtener estado de envío en texto
export const getEstadoEnvioTexto = (estado) => {
  const estados = {
    disponible: { texto: "Disponible", color: "text-green-500", bg: "bg-green-500/10" },
    transito: { texto: "En camino", color: "text-yellow-500", bg: "bg-yellow-500/10" },
    proximamente: { texto: "Próximamente", color: "text-blue-500", bg: "bg-blue-500/10" },
  };
...
export const obtenerDisponibles = () => {
  return perfumes.filter(p => p.estadoEnvio === "disponible");
};

// Función para obtener productos en tránsito
export const obtenerEnTransito = () => {
  return perfumes.filter(p => p.estadoEnvio === "transito");
};

// Función para obtener productos próximamente
export const obtenerProximamente = () => {
  return perfumes.filter(p => p.estadoEnvio === "proximamente");
};

// Función para obtener ofertas
export const obtenerOfertas = () => {
  return perfumes.filter(p => p.enOferta === true && p.estadoEnvio === "disponible");
};

// Función para filtrar por categoría
export const filtrarPorCategoria = (categoria) => {
  return perfumes.filter(p => p.categoria === categoria);
};

// Función para buscar perfumes
export const buscarPerfumes = (termino) => {
